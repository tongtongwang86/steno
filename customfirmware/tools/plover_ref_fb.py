...
def build(dict_path):
    ...
    # Force Plover onto its own no-word-list path so the comparison is
    # apples to apples: american_english_words.txt (4.3 MB) cannot ship.
    from plover import orthography as _o
    def _fallback(word, suffix):
        suffix, sep, rest = suffix.partition(' ')
        cands = _o.make_candidates_from_rules(word, suffix)
        return (cands[0] if cands else word + suffix) + sep + rest
    _o.add_suffix = _fallback
    import plover.meta.attach as _a
    _a.add_suffix = _fallback
